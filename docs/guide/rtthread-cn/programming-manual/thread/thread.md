...
struct rt_thread
{
    ...
    rt_thread_cleanup_t cleanup;       /* 线程退出清除函数 */
    struct rt_spinlock spinlock;       /* 自旋锁 */
    rt_ubase_t user_data;              /* 用户数据 */
};
```

其中线程优先级、状态和时间片等调度相关信息由 RT_SCHED_THREAD_CTX 展开管理。cleanup 会在线程退出时，被空闲线程回调一次以执行用户设置的清理现场等工作。最后的一个成员 user_data 可由用户挂接一些数据信息到线程控制块中，以提供一种类似线程私有数据的实现方式。
...
对于需要只关闭线程调度状态、并自行管理后续资源回收的内核代码或高级组件，RT-Thread 提供了更底层的线程关闭接口：

```c
rt_err_t rt_thread_close(rt_thread_t thread);
```

`rt_thread_close()` 会把线程从调度队列中移除，脱离线程定时器，并把线程状态置为 `RT_THREAD_CLOSE`。线程关闭接口 rt_thread_close() 的参数和返回值见下表：
...
需要注意的是，`rt_thread_close()` 不同于 `rt_thread_delete()` 和 `rt_thread_detach()`：该接口不会把关闭的线程加入 `rt_thread_defunct` 清理队列，也不会释放动态线程控制块和线程栈。因此应用层释放动态线程资源仍应使用 `rt_thread_delete()`，脱离静态线程仍应使用 `rt_thread_detach()`；`rt_thread_close()` 更适合内核或高级组件在已经自行管理后续生命周期时使用。
...
> [!NOTE]
> 注：rt_thread_yield()还做了上锁，修改当前线程状态的工作，最后调用了rt_schedule() 。rt_thread_yield()是面向用户的接口，rt_schedule()是内核实现调度的内部接口。一般情况下应调用rt_thread_yield()。
...
这两个函数接口的作用相同，调用它们可以使当前线程挂起一段指定的时间，当这个时间过后，线程会被唤醒并再次进入就绪状态。这个函数接受一个参数，该参数指定了线程的休眠时间。线程睡眠接口 rt_thread_delay/mdelay() 的参数和返回值见下表：
...
- RT_THREAD_CTRL_RESET_PRIORITY：重置线程优先级，arg 指向新的优先级值，类型为 `rt_uint8_t *`。

### 查询线程 CPU 使用率

开启 `RT_USING_CPU_USAGE_TRACER` 后，可以使用如下接口查询指定线程的 CPU 使用率：

```c
rt_uint8_t rt_thread_get_usage(rt_thread_t thread);
```

该接口按线程累计的 `user_time + system_time` 与系统各 CPU 累计的 `user + system + idle` 计算百分比，返回值范围为 0 到 100。`thread` 为需要查询的线程句柄。
...
usage = rt_thread_get_usage(thread);
rt_kprintf("%s cpu usage: %u%%\n", thread->parent.name, usage);
#endif
```

### 设置栈溢出处理钩子

开启 `RT_USING_OVERFLOW_CHECK` 且启用 hook 后，可以设置栈溢出处理钩子：

```c
void rt_scheduler_stack_overflow_sethook(rt_err_t (*hook)(struct rt_thread *thread));
```

当 `rt_scheduler_stack_check()` 检测到线程栈溢出时会调用该钩子，并把发生溢出的线程作为参数传入。钩子返回 `RT_EOK` 表示溢出已由钩子处理，内核不会进入默认的死循环；返回其他值或未设置钩子时，内核仍会按默认方式停止在溢出处理处。

使用示例：

```c
#if defined(RT_USING_OVERFLOW_CHECK) && defined(RT_USING_HOOK) && defined(RT_HOOK_USING_FUNC_PTR)
