...
### QEMU virt64 AArch64 启动流程

`bsp/qemu-virt64-aarch64` 展示了 AArch64 平台上的 SMP 启动方式。这个 BSP 使用 QEMU `virt` 机器模型，设备树中的 CPU 节点通过 `enable-method = "psci"` 描述唤醒方式，运行时由启动核把系统初始化完成后，再通过 PSCI 将次级核拉起。

![QEMU virt64 AArch64 多核启动总览图](figures/smp_boot_overview.svg)

启动核从 `libcpu/aarch64/cortex-a/entry_point.S` 中的 `_start` 进入。QEMU 固件跳转到内核入口时，`x0` 保存 DTB 物理地址；早期汇编代码会清理线程指针寄存器、统一异常级别、清零 BSS、准备启动栈，并调用 `rt_hw_fdt_install_early(x0)` 保存设备树地址。随后内核建立早期页表、打开 MMU，进入 `rtthread_startup()`。

进入 C 语言启动流程后，`rtthread_startup()` 会初始化调度器、系统定时器、对象系统、main 线程和 idle 线程。板级入口 `rt_hw_board_init()` 会进入 AArch64 通用初始化流程，完成异常向量表、内核地址空间、系统堆、设备树解析、GIC/GICv3、UART、全局 timer、SMP IPI 和 idle 钩子等基础设施初始化。

次级核的唤醒发生在 `main_thread_entry()` 早期调用的 `rt_hw_secondary_cpu_up()` 中。该函数遍历启动阶段解析出的 CPU 节点，根据设备树中的 `enable-method` 选择唤醒方式：

- `psci`：调用 PSCI `CPU_ON`，把 `_secondary_cpu_entry` 的物理地址传给固件。
- `spin-table`：写入 `cpu-release-addr`，再通过 `sev` 唤醒等待中的 CPU。

次级核从 `_secondary_cpu_entry` 进入后，会读取 `mpidr_el1` 并匹配 `rt_cpu_mpidr_table[]`，得到自己的逻辑 CPU 编号；随后准备每核独立栈、设置早期 MMU，并跳到 `rt_hw_secondary_cpu_bsp_start()`。该函数负责每核本地收尾，包括设置本核异常向量表、初始化 GIC CPU 接口或 GICv3 Redistributor、启动本地 generic timer、解除 SMP IPI 屏蔽、调用 `rt_dm_secondary_cpu_init()` 注册 CPU 相关设备，最后调用 `rt_system_scheduler_start()` 进入调度。

![QEMU virt64 AArch64 多核拉起时序图](figures/smp_boot_timeline.svg)

从时序上看，启动核先完成公共内核对象、页表、GIC、timer 和 IPI 等全局设施初始化，再运行 main 线程唤醒次级核；次级核补齐本地硬件状态后加入同一个 SMP 调度体系。如果多核没有正常启动，可以优先检查 QEMU 是否使用 `-machine virt` 和 `-smp N`，设备树 CPU 节点是否包含 `enable-method = "psci"`，`_secondary_cpu_entry` 是否能通过 `rt_kmem_v2p()` 转成有效物理地址，以及串口日志中是否有 `Call cpu X on success/failed` 相关提示。

关键代码位置如下：

| 阶段 | 代码位置 | 作用 |
| --- | --- | --- |
| 早期汇编入口 | `libcpu/aarch64/cortex-a/entry_point.S` | `_start`、`_secondary_cpu_entry`、早期 MMU |
| BSP 汇合入口 | `bsp/qemu-virt64-aarch64/drivers/board.c` | 将 `rt_hw_board_init()` 接到 AArch64 通用初始化 |
| AArch64 通用初始化 | `libcpu/aarch64/common/setup.c` | `rt_hw_common_setup()`、`rt_hw_secondary_cpu_up()`、`rt_hw_secondary_cpu_bsp_start()` |
| PSCI 支持 | `libcpu/aarch64/common/psci.c` | 通过固件执行 `CPU_ON` |
| 内核启动骨架 | `src/components.c` | `rtthread_startup()`、`main_thread_entry()` |
...
1. 如果它不是 CPU 绑定任务，则把它挂入全局就绪表，并向其它的所有 CPU 发送 IPI(Inter-Processor Interrupt 处理器间中断) 中断，通知它们检查是否需要切换任务，因为其它 CPU 的当前任务的优先级可能低于此就绪态任务，因而会发生优先级抢占；
