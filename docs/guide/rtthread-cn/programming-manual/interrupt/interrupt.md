...
程序状态字寄存器里保存算术与逻辑标志，例如负数标志，零结果标志，溢出标志等等。中断屏蔽寄存器组控制 Cortex-M 的中断使能。控制寄存器用来定义特权级别和当前使用哪个堆栈指针。
...
当架构开启 `ARCH_USING_IRQ_CTX_LIST` 时，RT-Thread 提供了一组中断 / 异常上下文记录接口，用于架构层维护当前 CPU 上的嵌套中断上下文链表：
...
其中 `rt_interrupt_context_t` 指向架构层准备的上下文记录节点，`context` 成员保存架构相关的中断或异常帧指针，`node` 成员用于挂入当前 CPU 的上下文链表。中断上下文记录接口的参数和返回值见下表：

| **函数** | **参数 / 返回值** | **描述** |
| -------- | ----------------- | -------- |
| rt_interrupt_context_push() | 参数 `this_ctx` | 把 `this_ctx` 指向的上下文记录压入当前 CPU 的上下文链表 |
| rt_interrupt_context_pop() | 无参数、无返回值 | 弹出当前 CPU 上最近一次压入的上下文记录 |
| rt_interrupt_context_get() | 返回当前上下文指针 | 返回当前 CPU 上下文链表头保存的架构相关上下文指针 |

该组接口主要由架构移植和异常处理代码使用，普通应用不需要直接调用。
...
所以通常情况下，实时系统中更多采用的是中断模式来驱动外设。当数据到达时，由中断唤醒相关的处理线程，再继续进行后续的动作。例如一些携带 FIFO（包含一定数据量的先进先出队列）的串口外设，其写入过程可以是这样的，如下图所示：
