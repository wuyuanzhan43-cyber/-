...
| name     | 对象的名字。每个对象可以设置一个名字，这个名字的最大长度由 RT_NAME_MAX 指定。默认值为 12，对象名称会按 RT_NAME_MAX - 1 个字符保存并补 `'\0'` |
...
推荐使用 `rt_object_for_each()` 遍历指定类型的内核对象。该接口在内部持有对象容器锁，遍历期间会依次调用用户传入的迭代回调函数：
...
|**参数**|**描述**|
|----------|------------|
| type     | 要遍历的对象类型，如 RT_Object_Class_Thread、RT_Object_Class_Mutex |
| iter     | 遍历回调函数 |
| data     | 传递给遍历回调函数的用户数据 |
|**返回**| —— |
| RT_EOK   | 遍历完成，或回调函数返回正数要求提前结束 |
| 负值错误码 | 回调函数返回的错误码，或对象类型非法 |

回调函数返回 `RT_EOK` 表示继续遍历；返回大于 0 的值表示提前结束且整体返回 `RT_EOK`；返回负值表示出错，`rt_object_for_each()` 会停止遍历并返回该错误码。该接口不能在中断上下文中调用。

以遍历所有线程为例：

```c
static rt_err_t print_thread_name(rt_object_t object, void *data)
{
...
rt_object_for_each(RT_Object_Class_Thread, print_thread_name, RT_NULL);
```

再以遍历所有互斥量为例：

```c
static rt_err_t print_mutex_name(rt_object_t object, void *data)
{
...
rt_object_for_each(RT_Object_Class_Mutex, print_mutex_name, RT_NULL);
```
...
```c
/* 表示内核对象名称的最大保存长度。默认值为 12，实际可用名称字符数为 RT_NAME_MAX - 1。*/
#define RT_NAME_MAX 12

/* 字节对齐时设定对齐的字节个数。常使用 rt_align(RT_ALIGN_SIZE) 进行字节对齐。*/
...
3）RT_UNUSED(x)，定义如下，表示函数或变量可能不使用，这个属性可以避免编译器产生警告信息。
...
5）rt_align(n)，定义如下，作用是在给某对象分配地址空间时，将其存放的地址按照 n 字节对齐，这里 n 可取 2 的幂次方。字节对齐的作用不仅是便于 CPU 快速访问，同时合理的利用字节对齐可以有效地节省存储空间。
