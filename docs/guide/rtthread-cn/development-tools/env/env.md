...
RT-Thread 的构建工具补充支持 `vsc_workspace` 目标，可用于生成 VS Code workspace：

```
scons --target=vsc_workspace
```

同时，构建系统新增 `package.json` 组件描述支持，简单组件可以用结构化 JSON 描述源码、头文件路径、宏定义和依赖；复杂组件仍可继续使用传统 `SConscript`。如果项目需要处理设备树文件，v5.2.x 还提供了 DTC 工具封装，可参考 [设备树 DTC 工具](/rt-thread-version/rt-thread-standard/programming-manual/device/ofw/dtc.md)。
