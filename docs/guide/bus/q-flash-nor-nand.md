...
answer: |
  **NOR Flash**：**可按字节/按位随机读**、可 XIP（在 Flash 上直接执行代码），读快、写慢、擦除粒度小、寿命较短、容量小（几 MB~几十 MB），成本高。适合**存固件/代码/启动代码**（Bootloader、Uboot）。
  **NAND Flash**：**按页读写、按块擦除**，**不能随机按字节读**、不能 XIP，**容量大、成本低、速度快、寿命长**，但需**坏块管理、ECC 纠错、磨损均衡**，且读需页缓冲。适合**大容量数据/文件系统**（rootfs、存量数据、eMMC/SD）。
