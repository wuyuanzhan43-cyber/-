...
- **目录树高亮 + 学习进度**：侧边栏高亮当前页/所在分类，并按分类点亮已学习进度（熟/生、掌握）。
- **分类树 + 本地全文搜索**：沿用验证过的嵌入式命题骨架（C / OS / Linux / 驱动 / ARM / 总线 / MCU / 工具链）。
...
docs/
  ...
  guide/
    readme.md                 # 手册说明
    c/  ds/  os/  rtos/  rtthread/  linux/  arm/  bus/  method/   # 分类目录（rtos=STM32+FreeRTOS 深挖；rtthread=RT-Thread 深挖）
